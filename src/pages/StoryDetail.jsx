...
import { getStory, mockStories } from '../data/mockStories'

const sectionMeta = [
  { key: 'happened', label: '起きたこと', tone: 'text-ink' },
  { key: 'hard', label: 'つらかったこと', tone: 'text-notice-700' },
  { key: 'tried', label: 'やってみたこと', tone: 'text-sage-700' },
  { key: 'worked', label: 'うまくいったこと', tone: 'text-warm-600' },
      ...
      <div className="px-5 pb-12 pt-4">
        ...
        <div className="mt-6 space-y-5">
          {sectionMeta.map(({ key, label, tone }) => (
            <section key={key}>
              <h2 className={`mb-1.5 text-sm font-bold ${tone}`}>
        ...
        <button
          ...
          className={`mt-8 flex w-full items-center justify-center gap-2 rounded-2xl border py-3.5 font-bold transition active:scale-[0.98] ${
            helped
              ? 'border-sage-500 bg-sage-500 text-white'
              : 'border-sage-200 bg-surface text-sage-700 hover:bg-sage-50'
          }`}
        >
